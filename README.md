...
| 模块 | 功能描述 |
|------|---------|
| 科普教具基础建档 | 编号、二维码值、科普品类、适配研学班级等信息管理 |
| 教职工责任人初始绑定 | 建档时分配教具专属负责人员 |
| 扫码核对更换责任人 | 先扫码核对档案，核对通过才允许登记新责任人 |
| 扫码核对记录留痕 | 每次核对（含失败）落记录，只增不改不删 |
| 按月自动汇总变更日志 | 生成月度变更台账总览，含扫码核对信息 |
| 按教职工反向查询 | 查询名下全部绑定教具 |

## 责任人更换流程（先扫码后换人）

1. **扫码核对**：更换时必须把这件教具标签上的二维码扫进来，系统按码值找到教具档案并核对。扫入的码与要更换的教具对不上、或该教具已不在原责任人名下，本次更换即不能成立，并明确告知原因。
2. **登记更换**：核对通过后生成限时（5 分钟）、一次性的核对凭证，凭凭证登记新责任人。未扫码直接提交、凭证已使用、凭证已超时，都会被拒绝并要求重新扫码。
3. **并发只落一次**：同一教具被同时转给不同人时，只有一笔能登记成功，另一笔会收到该教具已被转走的说明。
4. **全程留痕**：每次更换都能翻出扫的是哪个码、谁核对的、什么时候核对的；历史核对记录不可修改、不可删除。
...
责任人更换不再提供直改接口，必须走下方"扫码核对 → 登记更换"两步流程。

### 责任人更换（先扫码后换人）

| 方法 | 路径 | 描述 |
|------|------|------|
| POST | /api/kit-transfers/verify | 第一步：扫码核对，按码值核对教具档案，生成限时一次性核对凭证 |
| POST | /api/kit-transfers/confirm | 第二步：凭核对凭证登记更换，同一事务完成全部写入 |
| GET | /api/kit-transfers/verifications | 获取全部扫码核对记录（含失败记录） |
| GET | /api/kit-transfers/verifications/kit/{kitId} | 获取指定教具的扫码核对记录 |
